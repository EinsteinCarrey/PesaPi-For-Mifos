...
class configtool extends \PLUSPEOPLE\SlowTemplate\Template {
	...
	public function AJAXGetAccountList() {
		...
		foreach ($accounts AS $account) {
			...
			$slow->assign(array("DOMAIN" => $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"],
													"ID" => $account->getId(),
	...
	protected function credentialsOk() {
		...
		$dbId = mysqli_connect(
			$config->getConfig("DatabaseHostRead"),
			$config->getConfig("DatabaseUserRead"),
			$config->getConfig("DatabasePasswordRead"),
            $config->getConfig("DatabaseDatabaseRead"));
